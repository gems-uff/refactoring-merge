...
def analyse_branches(merge_commit,id_branch,refac_type_list, both_branches):
	...
	for sha1 in merge_commit['branch'+id_branch+'_list']:									
		if(sha1 in cache_commit_refactoring.items()): #ADDED 28/12/2021
		...
		if sha1 in cache_commit.items(): #It is a merge_commit ==> complement the list of commits in the branch
			#print("###### ACHOU MERGE ########")
			if both_branches:
...
def get_list_merge_commits(connection_bd, refac_type_list, both_branches):	
	with connection_bd.cursor() as cursor:
		cursor.execute("select c.id, c.sha1, c.date_time, mc.is_fast_forward_merge, mc.extra_effort, mc.wasted_effort, mc.rework_effort FROM commit c, merge_commit mc where c.id = mc.id_commit and mc.is_fast_forward_merge='False' and mc.has_base_version='True' order by c.date_time")
