...
def analyze(merge_commit, repo, output_file, normalized=False, collect=False):
	error = False
	commits_metrics = {}
	...
	try:
		...
		if(base):
			if(parent1.hex != base.hex and parent2.hex != base.hex):
				...
				if (collect):
					metrics.update(collect_attributes(diff_base_parent1, diff_base_parent2, base_version, parent1, parent2, repo, merge_commit))
						
					metrics['project'] = repo.workdir
					commits_metrics[merge_commit.hex] = metrics
	except:
		print()
		logger.exception("Unexpected error in commit " + str(merge_commit.hex))
		error = True
	...
	if(collect and not error):
		...
		merge_attr = []
		merge_attr.append(commits_metrics)
		write_json(merge_attr,"merge_commit_metrics.json")
...
def init_analysis(repo_path, output_file, normalized, collect, merge_commit):
	start_time = datetime.now()
	repo = Repository(repo_path)
	
	logger.info("Starting project" + repo.workdir)

	merge_commit = repo.get(merge_commit)
		
	analyze(merge_commit, repo, output_file, normalized, collect)
...
def main():
	...
	parser.add_argument("--local", help="set the path of a local git repository")
	...
	parser.add_argument("--mergecommit", help="set a merge commit sha1 to be analyzed")
	parser.add_argument("--output", default='output.csv', help="output file name. Default: output.csv")
	args = parser.parse_args()
	
	init_analysis(args.local, args.output, args.normalized, args.collect, args.mergecommit)
