...
projectDisc <- discretizeDF(project, methods = list(
  ...
  files_add_union = list(method = "fixed", breaks = c(0, 1, 2.5, 7.5, 23.5, 110.5,Inf), 
                         labels = c("nenhum", "muito pouco", "pouco","médio","muito", "bastante")),
  files_rm_b1 = list(method = "fixed", breaks = c(0, 1, 1.5, 4.5, 15.5, 82.5, Inf), 
                     labels = c("nenhum", "muito pouco", "pouco","médio","muito", "bastante")),
  files_rm_b2 = list(method = "fixed", breaks = c(0, 1, 1.5, 8.5, Inf), 
                     labels = c("nenhum", "pouco","médio","muito")),
  files_rm_intersection = list(method = "fixed", breaks = c(0, 1, 1.5, 8.5, Inf), 
                               labels = c("nenhum", "pouco","médio","muito")),
  files_rm_union = list(method = "fixed", breaks = c(0, 1, 1.5, 4.5, 15.5, 79.5, Inf), 
                        labels = c("nenhum", "muito pouco", "pouco","médio","muito", "bastante")),
  ...
  authors_b2 = list(method = "fixed", breaks = c(0, 1, 1.5, 3.5, Inf), 
                    labels = c("nenhum", "pouco","médio","muito")),
  authors_intersection = list(method = "fixed", breaks = c(0, 1, 1.5, 2.5, Inf), 
                              labels = c("nenhum", "pouco","médio","muito")),
  ...
  developer_commits_in_window_of_time = list(method = "fixed", breaks = c(0, 1, 20.5, 51.5, 98.5, 180.5, Inf), 
                                             labels = c("nenhum", "muito pouco", "pouco","médio","muito", "bastante")),
  developer_merges_in_window_of_time = list(method = "fixed", breaks = c(0, 1, 7.5, 19.5, 42.5, 104.5, Inf), 
                                            labels = c("nenhum", "muito pouco", "pouco","médio","muito", "bastante")),
  ...
  conflict_chunks = list(method = "fixed", breaks = c(0, 1, 1.5, 3.5, Inf), 
                           labels = c("nenhum", "pouco", "médio","muito")),
  conflict_files = list(method = "fixed", breaks = c(0, 1, 1.5, 2.5, Inf), 
                         labels = c("nenhum", "pouco", "médio","muito"))
) 
)
...
rules <- apriori(projectDisc, parameter = list(supp=0.0001, conf=0, target = "rules", maxtime=0,maxlen=2), appearance = list(rhs=c("wasted=nenhum", "wasted=pouco","wasted=muito","wasted=médio","rework=nenhum", "rework=pouco","rework=muito","rework=médio", "extra=nenhum", "extra=pouco","extra=muito","extra=médio")))

#rules <- apriori(projectDisc, parameter = list(supp=0.0001, conf=0, target = "rules", maxtime=0,maxlen=3), appearance = list(rhs=c("extra=nenhum", "extra=pouco","extra=muito","extra=médio")))

#rules <- apriori(projectDisc, parameter = list(supp=0.0001, conf=0, target = "rules", maxtime=0,maxlen=2), appearance = list(rhs=c("authors_intersection=nenhum", "authors_intersection=pouco","authors_intersection=muito","authors_intersection=médio")))
...
write(rulesWasted,
      file = "~/Documents/git/merge-effort-mining/result/rules_wasted_sup00001_conf_0.csv",
      sep = ",",
      quote = TRUE,
      row.names = FALSE)
...
write(rulesRework,
      file = "~/Documents/git/merge-effort-mining/result/rules_rework_sup00001_conf_0.csv",
      sep = ",",
      quote = TRUE,
      row.names = FALSE)
...
#rulesExtraAuthor <- sort(subset(rules, subset = lhs %pin% "extra"), by="lift") 
#inspect(rulesExtraAuthor)

write(rulesExtra,
      file = "~/Documents/git/merge-effort-mining/result/rules_extra_sup00001_conf_0_nenhum.csv",
      sep = ",",
      quote = TRUE,
      row.names = FALSE)
...
table(projectDisc$authors_intersection)
plot(projectDisc$authors_intersection)

print(project$extra)
print(project$branch2)


#identical(project$lines_changed_b1,project$branch1)
