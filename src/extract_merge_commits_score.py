...
def get_list_of_commit_in_branch(connection_bd, merge_commit, id_branch):
	...
	with connection_bd.cursor() as cursor:
		...
		cursor.execute("select commit.sha1 FROM commit, merge_branch where commit.id = merge_branch.id_commit and merge_branch.id_merge_commit=%s and merge_branch.type_branch = %s",parameters)
...
def calculate_qty_refactoring_each_commit_per_type(connection_bd, refac_type_list):		
	for refactoring in refac_type_list:		
		with connection_bd.cursor() as cursor:
			cursor.execute("select commit.sha1, refactoring.type, count(*) as qty from commit, refactoring where commit.id = refactoring.id_commit and refactoring.type = %s group by sha1", refactoring.replace("_"," "))
...
def get_list_of_distinct_refactoring(connection_bd):
	refactoring_list = []
	with connection_bd.cursor() as cursor:
...
def get_list_merge_commits(connection_bd, refac_type_list, both_branches):	
	with connection_bd.cursor() as cursor:
		cursor.execute("select commit.id, commit.sha1, commit.date_time, merge_commit.is_fast_forward_merge, merge_commit.extra_effort, merge_commit.wasted_effort, merge_commit.rework_effort FROM commit, merge_commit where commit.id = merge_commit.id_commit and merge_commit.is_fast_forward_merge='False' order by commit.date_time")
...
def init_analysis(both_branches=False):	
	start_time = datetime.now()	
	logger.info("Starting evaluation")	
	#try:
	connection_bd = open_connection_db()
	refac_type_list = get_list_of_distinct_refactoring(connection_bd)				
	...
	#	logger.info("Error: Another Error - " + str(ex))		
	#finally:		
	connection_bd.close()
...
def main():	
	...
	group.add_argument("--branches", action='store_true', help="boolean that indicate to split refactoring attributes in two branches")
	args = parser.parse_args()		
	init_analysis(args.branches)
